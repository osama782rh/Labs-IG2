...
3) To infer on an image that is stored on your local machine (run get_model.sh if it is the first time to test yolov3)
```
python3 yolo.py --image-path='/path/to/image/'
```
4) To infer on a video that is stored on your local machine (run get_model.sh if it is the first time to test yolov3)
```
